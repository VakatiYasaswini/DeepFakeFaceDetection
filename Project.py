...
sm = nn.Softmax()
inv_normalize= transforms.Normalize(mean=1*np.divide(mean,std),std=np.div
ide([1,1,1],std))
...
scroll_value,transform = train_transforms)
...
accuracy=accuracy, frame_path=frame_path, video_path= '.'+pathProvider,
face_path=face_path)
